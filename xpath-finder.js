...
function walkUpToClickable(el) {
  ...
  while (cur && depth < 6) {
    ...
    if (role === "button" || role === "menuitem" || role === "tab")
      return cur;
    if (t === "div" && cur.getAttribute("onclick"))
      return cur;
...
function gen(rawEl) {
  ...
  if (t === "a") {
    ...
    if (navP) {
      var navA = navP.getAttribute("aria-label") ||
        navP.getAttribute("data-label") ||
        navP.getAttribute("role");
      if (navA && txt)
        r.push("//*[@aria-label=" + wq(navA) +
          "]//a[text()=" + wq(txt) + "]");
    }
  }
  ...
  // Other leaf
  if ((ownTxt || txt) && el.children.length === 0 &&
  ...
  commonA.forEach(function (a) {
    ...
    if (a === "role" &&
        /^(presentation|none|group)$/.test(v)) return;
    r.push("//" + t + "[@" + a + "=" + wq(v) + "]");
  });
  ...
  while (anc && depth < 8) {
    ...
    for (var ai = 0; ai < ancA.length; ai++) {
      var av = anc.getAttribute &&
        anc.getAttribute(ancA[ai]);
      if (av) {
        r.push("//*[@" + ancA[ai] + "=" +
          wq(av) + "]//" + t);
        r.push("//*[@" + ancA[ai] + "=" +
  ...
  while (c && c.nodeType === 1 && maxP > 0) {
   ...
   if (window.__xf) return;
   window.__xf = true;
   var on = false, box = null, hoverEl = null, hoverOutline = "";
   ...
   function getDirectSpanText(el) {
     var spans = el.querySelectorAll(":scope > span");
     if (spans.length === 1 && spans[0].textContent.trim()) return spans[0].textContent.trim();
     if (spans.length === 0) { var ds = el.querySelector("span"); if (ds && ds.textContent.trim().length < 50) return ds.textContent.trim(); }
     return "";
   }
   ...
   function walkUpToClickable(el) {
     ...
     while (cur && depth < 6) {
       var t = cur.tagName ? cur.tagName.toLowerCase() : "";
       if (t === "button" || t === "a" || cur.getAttribute("role") === "button" || cur.getAttribute("role") === "menuitem" || cur.getAttribute("role") === "tab") return cur;
   ...
   function gen(rawEl) {
     ...
     if (t === "svg" || t === "path" || t === "use" || t === "circle" || t === "line" || t === "rect" || t === "polygon" || t === "g" || t === "img" || t === "i") {
       el = walkUpToClickable(rawEl);
       t = el.tagName ? el.tagName.toLowerCase() : "";
     }
     ...
     if (t.indexOf("-") > 0) {
       ["data-id", "data-name", "data-label", "data-tracking-type", "data-aura-class", "aria-label", "title", "name"].forEach(function (a) {
         var v = el.getAttribute(a);
       ...
       if (el.className && typeof el.className === "string") {
         ...
         for (var ci = 0; ci < cls.length; ci++) {
           if (cls[ci].length > 3 && !/^ng-|^_|^is-|^has-/.test(cls[ci])) {
             try { if (document.querySelectorAll(t + "." + CSS.escape(cls[ci])).length === 1) { r.push("//" + t + "[contains(@class," + wq(cls[ci]) + ")]"); break; } } catch (ex) {}
           }
         }
       }
     ...
     try {
       ...
       if (root && root !== document && root.host) {
         ...
         ["data-id", "data-name", "data-label", "data-tracking-type", "data-aura-class", "aria-label", "title"].forEach(function (a) {
           var hv = h.getAttribute(a);
     ...
     if (el.shadowRoot) {
       ["data-id", "data-name", "data-label", "data-tracking-type", "aria-label"].forEach(function (a) {
         var sv = el.getAttribute(a);
         if (sv) r.push("//" + t + "[@" + a + "=" + wq(sv) + "]");
       });
     }
     ...
     if (t === "button") {
       var bsTxt = getDirectSpanText(el);
       if (bsTxt) {
         r.push("//span[text()=" + wq(bsTxt) + "]/parent::button");
         r.push("//button[.//span[contains(text()," + wq(bsTxt) + ")]]");
       }
       if (el.getAttribute("title")) r.push("//button[@title=" + wq(el.getAttribute("title")) + "]");
       if (txt) {
         r.push("//button[contains(.," + wq(txt) + ")]");
         if (!bsTxt) r.push("//button[normalize-space()=" + wq(txt) + "]");
       }
       if (bsTxt && el.getAttribute("title")) r.push("//button[@title=" + wq(el.getAttribute("title")) + " and .//span[text()=" + wq(bsTxt) + "]]");
     }
     ...
     if (t === "a") {
       ...
       if (txt) { r.push("//a[text()=" + wq(txt) + "]"); r.push("//a[contains(text()," + wq(txt) + ")]"); r.push("//a[normalize-space()=" + wq(txt) + "]"); }
       if (el.getAttribute("title")) r.push("//a[@title=" + wq(el.getAttribute("title")) + "]");
       if (el.getAttribute("href")) {
         var href = el.getAttribute("href");
         if (href !== "#" && href !== "javascript:void(0)") {
           if (href.length < 60) r.push("//a[@href=" + wq(href) + "]");
           var hrefParts = href.split("/").filter(function (p) { return p && p.length > 2; });
           if (hrefParts.length > 0) r.push("//a[contains(@href," + wq(hrefParts[hrefParts.length - 1]) + ")]");
         }
       }
       var aSpan = getDirectSpanText(el);
       if (aSpan) r.push("//span[text()=" + wq(aSpan) + "]/parent::a");
       // Link inside a nav/list — use context
       var navParent = el.closest("nav, ul, [role='navigation'], [role='menubar'], [role='tablist']");
       if (navParent) {
         var navAttr = navParent.getAttribute("aria-label") || navParent.getAttribute("data-label") || navParent.getAttribute("role");
         if (navAttr && txt) r.push("//*[@aria-label=" + wq(navAttr) + "]//a[text()=" + wq(txt) + "]");
       }
     }
     ...
     if (t === "span") {
       ...
       if (sTxt && sTxt.length <= 50) {
         ...
         if (pt === "button") {
           ...
           r.push("//span[contains(text()," + wq(sTxt) + ")]/parent::button");
         } else if (pt === "a") {
           r.push("//span[text()=" + wq(sTxt) + "]/parent::a");
           r.push("//span[contains(text()," + wq(sTxt) + ")]/parent::a");
         } else {
           r.push("//span[text()=" + wq(sTxt) + "]");
           r.push("//span[contains(text()," + wq(sTxt) + ")]");
         }
     ...
     if (t === "div" && (ownTxt || txt) && el.children.length === 0) {
       ...
       if (dTxt.length <= 50) {
         var dp = el.parentElement;
         if (dp && dp.tagName.toLowerCase() === "a") r.push("//div[text()=" + wq(dTxt) + "]/parent::a");
         else r.push("//div[text()=" + wq(dTxt) + "]");
       }
     }

     // --- Other leaf text ---
     if ((ownTxt || txt) && el.children.length === 0 && !/^(button|a|span|div)$/.test(t)) {
       var lTxt = ownTxt || txt;
       if (lTxt.length <= 50) r.push("//" + t + "[text()=" + wq(lTxt) + "]");
     }

     // --- Radio / Checkbox ---
     if (t === "input" && el.type === "radio") { var rl = findLabel(el); if (rl) r.push("//span[text()=" + wq(rl) + "]/ancestor::label//input[@type='radio']"); }
     if (t === "input" && el.type === "checkbox") { var cl = findLabel(el); if (cl) r.push("//span[text()=" + wq(cl) + "]/ancestor::label//input[@type='checkbox']"); }
     ...
     // --- Common attributes ---
     ["data-label", "data-id", "data-name", "data-aura-class", "aria-label", "title", "name", "placeholder", "role", "type"].forEach(function (a) {
       var v = el.getAttribute(a);
       if (!v || v.length > 80) return;
       if (a === "type" && /^(text|hidden)$/.test(v)) return;
     ...
     // --- Stable ID ---
     if (el.id && /^[a-zA-Z][\w-]*$/.test(el.id) && !/\d{4,}/.test(el.id)) r.push("//*[@id='" + el.id + "']");
     ...
     if ((t === "input" || t === "textarea" || t === "select") && prevSib) {
       var prevTxt = prevSib.textContent ? prevSib.textContent.trim() : "";
       var prevTag = prevSib.tagName.toLowerCase();
       if (prevTxt && prevTxt.length < 40) {
         r.push("//" + prevTag + "[text()=" + wq(prevTxt) + "]/following-sibling::" + t);
         r.push("//" + prevTag + "[normalize-space()=" + wq(prevTxt) + "]/following-sibling::" + t);
       }
     }

     // following-sibling: element before a known sibling
     if (nextSib && txt) {
       var nextTxt = nextSib.textContent ? nextSib.textContent.trim() : "";
       var nextTag = nextSib.tagName.toLowerCase();
       if (nextTxt && nextTxt.length < 40 && nextTxt !== txt) {
         r.push("//" + nextTag + "[text()=" + wq(nextTxt) + "]/preceding-sibling::" + t);
       }
     }
     ...
     while (anc && depth < 8) {
       var aId = anc.id;
       var ancAttrs = ["data-label", "aria-label", "data-id", "data-name", "data-aura-class"];
       if (aId && /^[a-zA-Z][\w-]*$/.test(aId) && !/\d{4,}/.test(aId)) {
         r.push("//*[@id='" + aId + "']//" + t);
         r.push("//*[@id='" + aId + "']/descendant::" + t);
         break;
       }
       var found = false;
       for (var ai = 0; ai < ancAttrs.length; ai++) {
         var av = anc.getAttribute && anc.getAttribute(ancAttrs[ai]);
         if (av) {
           r.push("//*[@" + ancAttrs[ai] + "=" + wq(av) + "]//" + t);
           r.push("//*[@" + ancAttrs[ai] + "=" + wq(av) + "]/descendant::" + t);
           found = true; break;
         }
       }
       if (found) break;
     ...
     if (fullTxt.length > 15 && fullTxt.length <= 80 && el.children.length === 0) {
       r.push("//" + t + "[contains(text()," + wq(fullTxt.substring(0, 20)) + ")]");
     }

     // --- Children drill-down ---
     if (r.length < 2) {
       var inner = el.querySelector("a,button,input,select,textarea,span[onclick],div[role='button']");
       if (inner) gen(inner).forEach(function (item) { if (item.xp) r.push(item.xp); });
     }
   ...
   function findLabel(el) {
     if (el.id) { try { var lbl = document.querySelector('label[for="' + CSS.escape(el.id) + '"]'); if (lbl) { var s = lbl.querySelector("span"); return (s && s.textContent.trim()) || lbl.textContent.trim(); } } catch (e) {} }
     var w = el.closest("label,.slds-form-element,lightning-input,lightning-combobox,lightning-checkbox-group,lightning-radio-group,lightning-textarea,lightning-datepicker,lightning-input-field,lightning-select");
     if (w) { var wl = w.querySelector("span.slds-form-element__label,label span,legend span,label,.slds-form-element__legend"); if (wl && wl.textContent.trim()) return wl.textContent.trim(); }
   ...
   function findBestTarget(el, e) {
     var deep = el;
     try { var path = e.composedPath && e.composedPath(); if (path && path.length > 0) { for (var pi = 0; pi < path.length; pi++) { if (path[pi].nodeType === 1 && path[pi].tagName) { deep = path[pi]; break; } } } } catch (ex) {}
     if (deep === el) { try { var fp = document.elementFromPoint(e.clientX, e.clientY); if (fp && fp !== el) deep = fp; } catch (ex) {} }

     var t = deep.tagName ? deep.tagName.toLowerCase() : "";

     // SVG/icon → walk up
     if (t === "svg" || t === "path" || t === "use" || t === "circle" || t === "line" || t === "g" || t === "img" || t === "i") {
     ...
     if ((t === "div" || t === "nav" || t === "li" || t === "ul" || t === "section" || t === "header") && deep.children.length > 0) {
       var links = deep.querySelectorAll("a,button,[role='button'],[role='menuitem'],[role='tab']");
       if (links.length === 1) return links[0];
       if (links.length > 1) {
         var best = null, bd = Infinity;
         links.forEach(function (lnk) { var rect = lnk.getBoundingClientRect(); var d = Math.sqrt(Math.pow(e.clientX - (rect.left + rect.width / 2), 2) + Math.pow(e.clientY - (rect.top + rect.height / 2), 2)); if (d < bd) { bd = d; best = lnk; } });
         if (best) return best;
       }
     }
     if (!isKnown && t.indexOf("-") > 0) return deep;
     return deep;
   }
   ...
   function onHover(e) {
     ...
     if (hoverEl && hoverEl !== el) { hoverEl.style.outline = hoverOutline; }
     hoverEl = el;
   ...
   function show(el, e) {
     ...
     var elAttrs = "";
     ["id", "data-label", "aria-label", "title", "name", "class", "href"].forEach(function (a) {
       var v = el.getAttribute(a);
       if (v && !elAttrs) elAttrs = a + "=" + v.substring(0, 30);
     });

     box = document.createElement("div");
     box.id = "__xf_box";
     box.style.cssText = "position:fixed;z-index:999999;background:#1e1e1e;color:#d4d4d4;padding:12px 14px;border-radius:8px;font:12px monospace;max-width:680px;box-shadow:0 4px 16px rgba(0,0,0,0.5);";
     ...
     title.style.cssText = "font-weight:bold;margin-bottom:4px;color:#4fc3f7;";
     title.textContent = "XPath (" + results.length + ")  <" + elTag + ">";
     box.appendChild(title);

     var info = document.createElement("div");
     info.style.cssText = "margin-bottom:8px;font-size:11px;color:#90a4ae;";
     info.textContent = (elTxt ? '"' + elTxt + '"' : "(no text)") + (elAttrs ? "  |  " + elAttrs : "");
     box.appendChild(info);

     results.forEach(function (item, i) {
       var row = document.createElement("div");
       row.style.cssText = "margin-bottom:6px;display:flex;align-items:start;gap:6px;";
       var num = document.createElement("span"); num.style.cssText = "color:#aaa;min-width:14px;"; num.textContent = (i + 1) + ".";
       ...
       badge.textContent = item.count === 1 ? "unique" : item.count === 0 ? "0 hits" : item.count + " hits";
       ...
       function makeCopier(xpath, elem) { return function (ev) { ev.stopPropagation(); ev.preventDefault(); if (navigator.clipboard) navigator.clipboard.writeText(xpath).then(function () { elem.style.color = "#4caf50"; setTimeout(function () { elem.style.color = ""; }, 600); }); }; }
       code.onclick = makeCopier(item.xp, code); cpBtn.onclick = makeCopier(item.xp, cpBtn);
     ...
     if (isShadow) {
       ...
       warn.textContent = "Shadow DOM detected. XPaths target the host element.";
       box.appendChild(warn);
     }

     var hint = document.createElement("div");
     hint.style.cssText = "margin-top:6px;font-size:10px;color:#777;";
     hint.textContent = "Click XPath to copy | Green=unique | Red=multiple | Ctrl+Shift+X to toggle";
   ...
   function toggle() {
     ...
     if (on) {
       ...
       btn.textContent = "XPath: ON (Ctrl+Shift+X)"; btn.style.background = "#4caf50";
     } else {
       document.removeEventListener("click", handler, true);
       document.removeEventListener("mouseover", onHover, true);
       document.removeEventListener("mouseout", onHoverOut, true);
       hide();
       if (hoverEl) { hoverEl.style.outline = hoverOutline; hoverEl = null; }
       btn.textContent = "XPath: OFF (Ctrl+Shift+X)"; btn.style.background = "#f44336";
     }
   }
   ...
   document.addEventListener("keydown", function (e) {
     if ((e.ctrlKey || e.metaKey) && e.shiftKey && (e.key === "X" || e.key === "x")) { e.preventDefault(); toggle(); }
   });
   })();
...
function findBestTarget(el, e) {
  ...
  if (!isKnown) {
    ...
    if (inner) return inner;
    if (deep.shadowRoot) {
      var si = deep.shadowRoot.querySelector(
        "a,button,span,input"
      );
      if (si) return si;
    }
  ...
  if (/^(div|nav|li|ul|section|header)$/.test(t) &&
...
function onHover(e) {
  ...
  var el = e.target;
  if (el.closest &&
      el.closest("#__xf_box,#__xf_toggle")) return;
...
function show(el, e) {
  ...
  var elAttrs = "";
  [
    "id", "data-label", "aria-label",
    "title", "name", "class", "href"
  ].forEach(function (a) {
    var v = el.getAttribute(a);
    if (v && !elAttrs)
  ...
  var isShadow = false;
  try {
    var rt = el.getRootNode && el.getRootNode();
    if ((rt && rt !== document && rt.host) ||
        el.shadowRoot) isShadow = true;
  } catch (ex) {}
  if (isShadow) {
    var warn = document.createElement("div");
    warn.style.cssText =
      "margin-top:6px;padding:6px 8px;" +
      "background:#4a2800;border-radius:4px;" +
      "color:#ffb74d;font-size:11px;";
    warn.textContent =
      "Shadow DOM detected. XPaths target the host.";
    box.appendChild(warn);
  }
  ...
  hint.textContent =
    "Click XPath to copy | Green=unique | " +
    "Red=multiple | Ctrl+Shift+X to toggle";
  box.appendChild(hint);
  document.body.appendChild(box);
  el.style.outline = "3px solid #4fc3f7";
  setTimeout(function () {
...
function handler(e) {
  if (e.target.closest &&
      e.target.closest("#__xf_box")) return;
  if (e.target.id === "__xf_toggle") return;
  e.preventDefault();
  e.stopPropagation();
  show(e.target, e);
...
btn.style.cssText =
  ...
  "z-index:999998;padding:8px 16px;" +

...
> **Primary approach: navigate by href.** The `<a>` always carries a real
> `href` (`/lightning/r/...`), but SPA clicks on `records-hoverable-link` are
> flaky. So we read the inner anchor's **absolute href** and `driver.get(href)` —
> this loads the record page reliably every time. A `.click()` is only a safety net.
...
public void clickRecordLink(String row, String field, String screenName) {
    try {
        ...
        // Read the ABSOLUTE href from the inner <a> (inside the shadow root)
        String href = (String) ((JavascriptExecutor) driver).executeScript(
            ...
            "el.dispatchEvent(new MouseEvent('mouseover',{bubbles:true}));" +   // render hoverable link
            "var a = f(el);" +
            "return a ? a.href : '';",
            link);

        if (href != null && !href.isEmpty()) {
            driver.get(href);          // navigate directly — reliable
        } else {
            link.click();              // safety net if no href found
        }
        Thread.sleep(1500);
    } catch (Exception e) {
        logger.info(e);
